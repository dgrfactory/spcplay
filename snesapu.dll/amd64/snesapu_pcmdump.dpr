{===================================================================================================
 ...
 pattern (same SetAPUOpt settings, same chunk size per EmuAPU call, same total sample count). Run this
 once against snesapu.dll (x86 build) and once against snesapu_amd64.dll (x64 build) with identical
 arguments, and the two output files should be byte-for-byte identical. Any difference is a real
 behavioral divergence between the two builds, not a rendering/rounding artifact -- there is no
 audio device, resampling, or floating-point display step in between to blur a comparison.
 ...
 path argument at runtime must match whichever this EXE was built for):
   fpc -Pi386   -Twin32 snesapu_pcmdump.dpr      (produces the x86 tester, to run against snesapu.dll)
   fpc -Px86_64 -Twin64 snesapu_pcmdump.dpr      (produces the x64 tester, to run against snesapu_amd64.dll)
 ...
 Example (from cmd, comparing the two builds against the same .spc):
   snesapu_pcmdump_x86.exe  ..\snesapu.dll\snesapu.dll        song.spc out_x86.pcm 1000000
   snesapu_pcmdump_x64.exe  ..\snesapu_amd64.dll\snesapu_amd64.dll song.spc out_x64.pcm 1000000
   fc /b out_x86.pcm out_x64.pcm

 SetAPUOpt is called with fixed values below (mixType=1, 2 channels, 16-bit, 32000Hz, INT_GAUSS,
 opts=0) -- these must NOT be changed between the two runs being compared, since the output format
 and mixing depend on them. EmuAPU is called in fixed CHUNK_SAMPLES-sized chunks rather than one
 huge request, both because that's how a real frontend calls it and to keep memory use bounded; the
 chunk size itself doesn't need to match spcplay.exe's, only between the two runs being compared to
 each other (SPC700/DSP.asm state advances the same way per sample regardless of how the caller
 batches its EmuAPU calls, so any *fixed* chunk size is fine as long as it's the same on both sides).
===================================================================================================}
